    ...
    css: {
      ...
      files: [
        ...
        },
        {
          ...
          format: 'css/variables',
          filter: (token) => {
            // Debug: log dark theme tokens
            if (token.filePath && token.filePath.includes('dark')) {
              console.log('Dark token:', token.name, 'path:', token.path, 'filePath:', token.filePath);
            }
            return token.filePath && token.filePath.includes('themes/dark');
          },
          options: {
            outputReferences: true,
